...
class PostcardCrudTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_index_loads_with_200(): void
    {
        ...
        $response = $this->get(route('admin.postcards.index'));
    ...
    public function test_create_form_loads_with_200(): void
    {
        ...
        $response = $this->get(route('admin.postcards.create'));

        $response->assertOk();
    }

    public function test_store_creates_postcard_with_minimal_fields(): void
    {
        $this->actingAs($this->makeAdminUser());

        $response = $this->post(route('admin.postcards.store'), [
            'year' => 1943,
        ]);

        $response->assertRedirect();
        $this->assertDatabaseHas('postcards', ['year' => 1943]);
    }
    ...
    public function test_store_accepts_inline_image_upload(): void
    {
        ...
        $response->assertRedirect();
        $postcard = Postcard::latest('id')->firstOrFail();
        $media = $postcard->media()->where('collection', 'images')->first();
    ...
    public function test_update_changes_fields(): void
    {
        ...
        $postcard = Postcard::create(['year' => 1940]);

        $response = $this->put(route('admin.postcards.update', $postcard), [
            'year' => 1945,
        ]);
